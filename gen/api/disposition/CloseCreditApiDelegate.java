...
/**
 * A delegate to be called by the {@link CloseCreditApiController}}.
...
public interface CloseCreditApiDelegate {
     ...
     *         or Internal Server Error  Недоступность сервера  Иные неучтенные ошибки (status code 500)
     * @see CloseCreditApi#deleteDispositionCloseCreditDetailsById
     */
    default ResponseEntity<Void> deleteDispositionCloseCreditDetailsById(UUID id) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "{ \"code\" : 400, \"description\" : \"Internal Server Error\", \"timestamp\" : 1557057600 }";
