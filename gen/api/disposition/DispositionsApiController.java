...
public class DispositionsApiController implements DispositionsApi {

    private final DispositionsApiDelegate delegate;

    public DispositionsApiController(@Autowired(required = false) DispositionsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new DispositionsApiDelegate() {});
    }

    @Override
    public DispositionsApiDelegate getDelegate() {
        return delegate;
    }
